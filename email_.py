...
# mag=MIMEText("yeume","html","utf-8")
# mag["from"]=formataddr("cuiyifeng","1797269751@qq.com")
#
# mag["Subject"]="ddkd"
#
#
# server=smtplib.SMTP("smtp.qq.com")
# server.login("1797260751@qq.com","zdsgqgsnukplceab")
# server.sendmail("1797269751@qq.com","1955632351",mag.as_string())
# server.quit()
import smtplib
...
def send_email(smtpHost,port, sendAddr, password, recipientAddrs, subject='', content=''):
    ...
    part = MIMEApplication(open(r'C:\Users\86189\Desktop\day13【单元测试】\代码\day13\计算器.html','rb').read())
    part.add_header('Content-Disposition', 'attachment', filename="计算器.html")#给附件重命名,一般和原文件名一样,改错了可能无法打开.
    msg.attach(part)
...
if __name__ == "__main__":
    try:
        ...
        port = 465 #端口号
        sendAddr ='1797269751@qq.com'#发送方地址
        password = input("请输入邮箱授权码：")#手动输入授权码更安全.授权码的获取:打开qq邮箱->设置->账户->开启IMAP/SMTP服务->发送短信->授权码
        recipientAddrs = '1797269751@qq.com;1955632351@qq.com'#接收方可以是多个账户, 用分号分开,send_email()函数中手动设置
        subject='python test from yangjian'#主题
        content='附件是测试报告'#正文内容
